...
export const singUp = async (req, res) => {
    try {
        ...
        if(!name || !email || !password) return res.status(400).json({error: "Fields can not be empty"})
...
export const logIn = async (req, res) => {
    try {
        ...
        if(!email || !password) return res.status(400).json({error: "Fields can not be ampty"});
